...
appcenter test run uitest --app $APPCENTER_PROJECT_NAME --devices $DEVICES --app-path $APPCENTER_OUTPUT_DIRECTORY/com.microsoft.tailwindtraders.ipa  --test-series "master" --locale "en_US" --build-dir $APPCENTER_SOURCE_DIRECTORY/Source/SmartHotel.Clients.UITests/bin/Release --uitest-tools-dir $UI_TEST_TOOLS_DIR --token $APPCENTER_TOKEN --async
